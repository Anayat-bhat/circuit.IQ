    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        if (meshRef.current) {
          ...
          let mixedColor = dColor.clone().lerp(gColor, intensity);
          if (isHovered) {
            mixedColor = new THREE.Color('#ffffff').lerp(gColor, 0.25);
          }
          meshRef.current.setColorAt(index, mixedColor);
        }
        ...
        if (coreMeshRef.current) {
          ...
          let mixedCoreColor = dColor.clone().lerp(cColor, intensity);
          if (isHovered) {
            mixedCoreColor = new THREE.Color('#ffffff');
          }
          coreMeshRef.current.setColorAt(index, mixedCoreColor);
        }
        ...
        if (haloMeshRef.current) {
          ...
          let mixedHaloColor = dColor.clone().lerp(gColor, intensity);
          if (isHovered) {
            mixedHaloColor = new THREE.Color('#ffffff').lerp(gColor, 0.3);
          }
          mixedHaloColor.multiplyScalar(intensity * 1.8);
          haloMeshRef.current.setColorAt(index, mixedHaloColor);
        }
    ...
    <div className="w-full flex flex-col xl:flex-row gap-6 bg-slate-950/85 p-6 md:p-8 rounded-3xl border border-white/5 backdrop-blur-3xl shadow-[0_24px_70px_rgba(0,0,0,0.8)] relative overflow-hidden transition-all duration-300">
      ...
      >
        ...
        <Canvas shadows>
          ...
          {/* Main direction spotlight casting beautiful soft shadows */}
          <spotLight
            position={[5, 10, 5]}
            angle={0.4}
            penumbra={0.9}
