...
export default function LandingPage() {
  ...
  return (
    <div ref={scrollRef} className="relative min-h-[500vh] bg-space-black">
      <AntigravityHero />
      ...
      <section className="sticky top-0 h-screen flex flex-col items-center justify-center text-center px-6 overflow-hidden">
        ...
        >
          ...
          <p className="max-w-2xl mx-auto text-2xl text-slate-500 font-light italic mb-12 leading-relaxed">
            AI-Powered Virtual Physics Laboratory for the <span className="text-blue-600">Modern Engineer</span>.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <button 
              onClick={() => setLabOpen(true)}
              className="px-10 py-4 bg-slate-900 text-white font-bold rounded-xl flex items-center gap-2 hover:bg-slate-800 transition-all hover:scale-105 active:scale-95 shadow-xl shadow-slate-900/10"
            >
              Start Experimenting
              <Zap size={20} fill="currentColor" />
            </button>
            <button className="px-10 py-4 bg-transparent border border-slate-300 text-slate-900 font-bold rounded-xl hover:bg-slate-100 transition-all">
              Explore Library
            </button>
          </div>
        </motion.div>
        ...
        >
          <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Scroll to Build</span>
          <MousePointer2 size={16} className="text-slate-500" />
        </motion.div>
      </section>

      {/* Assembly Section */}
      <section className="relative h-[200vh] flex flex-col items-center justify-center pointer-events-none">
         <motion.div 
          ...
          >
            <div>
              <h2 className="text-4xl font-display font-bold mb-6">Real-time Simulation</h2>
            ...
            <div className="relative rounded-2xl overflow-hidden border border-slate-200 aspect-video bg-blue-50/50 flex items-center justify-center">
      ...
      <section className="relative px-6 py-32 max-w-7xl mx-auto">
          <div className="text-center mb-20">
            <h2 className="text-5xl font-display font-bold mb-4">Physics Domains</h2>
            <p className="text-slate-500 max-w-2xl mx-auto">Explore our extensive library of interactive physics experiments designed for students and enthusiasts.</p>
          </div>
      ...
      <section className="py-32 flex flex-col items-center justify-center text-center">
          <h2 className="text-6xl font-display font-bold mb-8">Ready to Experiment?</h2>
          <button 
            onClick={() => setLabOpen(true)}
            className="px-12 py-5 rounded-full bg-slate-900 text-white font-bold text-lg hover:scale-110 transition-all shadow-xl shadow-slate-900/10"
          >
            Enter Virtual Laboratory
          </button>
      </section>

      <footer className="py-12 border-t border-slate-200 text-center text-slate-500 text-[10px] font-mono tracking-[0.3em] uppercase relative z-10">
...
function CategoryCard({ title, icon, items }: { title: string, icon: React.ReactNode, items: string[] }) {
  return (
    <div className="glass-panel p-8 group hover:border-blue-500/30 transition-colors cursor-pointer bg-white/80">
      <div className="text-blue-600 mb-6 transform group-hover:scale-110 transition-transform duration-300">{icon}</div>
