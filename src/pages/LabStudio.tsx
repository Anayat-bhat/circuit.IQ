...
export default function LabStudio() {
    ...
    return (
        <div className="fixed inset-0 pt-20 bg-white flex flex-col z-10">
            {/* Toolbar */}
            <div className="h-16 border-b border-slate-200 px-6 flex items-center justify-between bg-white !rounded-none">
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2">
                        <span className="text-slate-500 text-[10px] font-mono uppercase tracking-widest">Experiment</span>
                        <select 
                            value={currentExperiment || 'ohms-law'} 
                            onChange={(e) => setCurrentExperiment(e.target.value)}
                            className="bg-transparent text-slate-900 font-medium focus:outline-none cursor-pointer hover:text-blue-600 transition-colors text-sm"
                        >
                            {experiments.map(exp => (
                                <option key={exp.id} value={exp.id} className="bg-white text-slate-900">{exp.name}</option>
                            ))}
                        </select>
                    </div>
                    <div className="h-6 w-px bg-slate-200" />
                    <div className="flex items-center gap-4">
                        <button className={cn("p-2 rounded-lg transition-all", tool === 'move' ? "bg-blue-600 text-white" : "text-slate-500 hover:text-slate-900 bg-slate-100")} onClick={() => setTool('move')}><MousePointer2 size={18}/></button>
                        <div className="h-6 w-px bg-slate-200" />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button 
                        ...
                        className={cn(
                            ...
                            physicsBotOpen ? "bg-blue-600 text-white border-blue-400/30 shadow-[0_0_15px_rgba(37,99,235,0.4)]" : "bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200 hover:text-slate-900"
                        )}
                    >
                        <Bot size={18}/>
                        PhysicsBot
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <aside className="w-20 border-r border-slate-200 flex flex-col items-center py-6 gap-6 bg-slate-50 overflow-y-auto">
                ...
                <main className="flex-1 relative bg-slate-50/50 overflow-hidden">
                    <div className="absolute top-4 left-6 text-[10px] font-mono text-slate-500 uppercase tracking-widest z-10">
                    ...
                    <div className="absolute bottom-6 left-6 px-4 py-2 bg-white border border-slate-200 rounded-2xl shadow-lg flex items-center gap-4 z-10">
                        <div className="flex items-center gap-2">
                           <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                           <span className="text-[10px] font-mono text-slate-600">SIMULATION ENGINE: READY</span>
                        </div>
                        <div className="w-px h-4 bg-slate-200" />
                        <span className="text-[10px] font-mono text-slate-400">CPU LOAD: 12%</span>
                    </div>
                    ...
                    <AnimatePresence>
                        {selectedComponent && (
                            <motion.div 
                                ...
                                exit={{ opacity: 0, x: 20 }}
                                className="absolute right-8 top-8 w-64 bg-white border border-slate-200 p-6 z-20 rounded-3xl shadow-xl"
                            >
                                <h3 className="font-display font-bold text-lg mb-4 text-slate-900">Resistor R1</h3>
                                ...
                                <button 
                                    className="w-full mt-6 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors flex items-center justify-center gap-2 font-medium"
                                    onClick={() => {
                                ...
                                <button className="absolute top-4 right-4 text-slate-400 hover:text-slate-600" onClick={() => setSelectedComponent(null)}>
                                    ✕
                                </button>
                            </motion.div>
                        )}
                ...
                <AnimatePresence>
                    {physicsBotOpen && (
                        <motion.aside 
                            ...
                            animate={{ x: 0 }}
                            exit={{ x: 400 }}
                            className="w-80 border-l border-slate-200 bg-slate-50 flex flex-col"
                        >
                            <div className="p-6 border-b border-slate-200 bg-white">
                                <div className="flex items-center gap-3 mb-4">
                                    <div className="w-3 h-3 rounded-full bg-green-500 shadow-[0_0_10px_#22c55e]"></div>
                                    <span className="text-[10px] font-bold text-slate-500 tracking-[0.2em]">PHYSICSBOT ACTIVE</span>
                                </div>
                                <h4 className="font-bold text-slate-900">Assistant</h4>
                            </div>
                            
                            <div className="flex-1 p-6 overflow-y-auto space-y-4">
                                <div className="p-3 rounded-xl bg-blue-50 text-[11px] text-slate-700 italic border-l-2 border-blue-500 leading-relaxed shadow-sm">
                            ...
                            <div className="p-4 border-t border-slate-200 mt-auto bg-white">
                                <div className="relative group">
                                    ...
                                    <input 
                                        ...
                                        className="relative w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-blue-500/50 focus:bg-white transition-colors text-slate-900 shadow-sm"
                                    />
                                </div>
                            </div>
                        </motion.aside>
...
function SidebarTool({ icon, active }: { icon: React.ReactNode, active?: boolean }) {
    return (
        <button className={cn(
            ...
            active ? "bg-white text-blue-600 border border-blue-200 shadow-md transform scale-105" : "bg-white text-slate-500 hover:text-slate-900 hover:bg-slate-50 border border-slate-200"
        )}>
            {icon}
        </button>
    );
    ...
    return (
        <button 
            ...
            className={cn(
                ...
                active ? "bg-blue-50 text-blue-600 border border-blue-200 shadow-sm" : "text-slate-600 hover:text-slate-900 hover:bg-slate-100 border border-transparent"
            )}
        >
            {icon}
            {label}
